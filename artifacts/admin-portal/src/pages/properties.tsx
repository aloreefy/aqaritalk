...
export default function Properties() {
  ...
  // We fetch without explicit status if 'all', else pass the status.
  // The hook signature accepts AdminListPropertiesParams
  const queryParams = {
    page,
    limit: 50,
  ...
  const handleStatusChange = (id: string, newStatus: 'active' | 'rejected' | 'deleted') => {
    updateStatusMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast({ title: "Asset record updated", description: `Asset status changed to ${newStatus}.` });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Monitor and moderate platform real estate listings.</p>
        </div>
      ...
      <div className="bg-card border rounded-lg overflow-hidden shadow-sm">
              ...
              {properties.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-muted-foreground">
                    ...
                    <p>No assets found matching current parameters.</p>
                  </td>
                </tr>
              ) : (
                properties.map((property) => (
                  <tr key={property.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[10px] text-muted-foreground mt-0.5">By: {property.createdBy.substring(0, 8)}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        ...
                        <span className="text-xs truncate max-w-[150px]">
                          {[property.district, property.city, property.country].filter(Boolean).join(', ') || 'Unspecified'}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                          ...
                          {property.status !== 'deleted' && (
                            <>
                              ...
                              <DropdownMenuItem onClick={() => handleStatusChange(property.id, 'deleted')} className="text-destructive cursor-pointer">
                                <Trash2 className="w-4 h-4 mr-2" /> Mark Deleted
                              </DropdownMenuItem>
                            </>
                          )}
                        </DropdownMenuContent>
        ...
        {data?.total && data.total > 50 && (
          <div className="p-4 border-t bg-muted/20 flex justify-between items-center text-xs text-muted-foreground">
            <span>Showing {properties.length} of {data.total} assets</span>
